...
public:
	...
	void EquipHp()
	{
		...
		for (const auto& eqItemPtr : inventory.equippedItems)
		{
			if (eqItemPtr != nullptr && eqItemPtr->id != -1)
			{
				...
				std::cout << "[장비 효과] 아이템 ID: " << eqItemPtr->id << " 체력 +" << totalEquipHpBonus << "\n";
			}
		}

		// 장비 보너스와 적용할 턴 수를 넣어 버프 갱신
		ApplyHpBuff(totalEquipHpBonus, 3);  // 예: 3턴 적용
	}
	...
	void UpdateBuff()
	{
		...
		if (currentBuff.hpBuffTurns > 0) {
			...
			if (healAmount > 0) {
				HealHp(healAmount);
				std::cout << "[회복] 체력이 " << healAmount << "만큼 회복되었습니다.\n";
			}
